...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/note') {
    ...
    if (!sessionId) return send(res, 400, { error: 'sessionId required' });
    if (!userPrompt && !aiResponse && !annotation) return send(res, 400, { error: 'provide at least one of userPrompt, aiResponse, annotation' });
    const session = db.getSession(sessionId);
    if (!session) return send(res, 404, { error: 'session not found' });
  ...
  if (req.method === 'GET' && url.pathname === '/stats') {
    ...
    const avgCompression = tokenStats.original > 0 ? Number(((tokenStats.saved / tokenStats.original) * 100).toFixed(2)) : 0;
    return send(res, 200, {
  ...
  if (req.method === 'GET' && url.pathname === '/ui') {
    ...
    try {
      ...
      return send(res, 500, { error: 'failed to read ui' });
    }
    return;
  }
...
server.listen(port, host, () => {
  console.error(`[HTTP] antigravity-memory API on http://${host}:${port} (mock=${process.env.MOCK_GEMINI === '1'})`);
});
