...
## 17. Релиз версии v0.4.0 (2026-07-22)

По результатам глубокого код ревью (3 независимых субагента + верификация) выполнен пакет исправлений и обновлена версия до **`0.4.0`**:

### Ключевые улучшения:
1. **FTS5 Unicode & Cyrillic Support (`src/core/database.ts`)**: Очистка поискового промпта переведена на `/[^\p{L}\p{N}\s]/gu`. Заметки и сессии на русском языке теперь без проблем находятся через полнотекстовый поиск.
2. **Безопасность AGY CLI (`src/gemini/agy-client.ts`)**: Учтена передача `this.modelName` и добавлено автоматическое усечение промптов длиннее 40 000 символов для исключения падений процесса с ошибкой ОС `E2BIG`.
3. **Отмена двойного JSON-экранирования (`src/core/database.ts`)**: `updateObservationResult` теперь обрабатывает сырые строки без добавления избыточных экранирующих кавычек `\"`.
4. **Индексы и Инкапсуляция SQLite**: Добавлены индексы `idx_sessions_project_status`, `idx_observations_session`, `idx_notes_session` и публичный метод `db.getStats()` для отказа от избыточных вызовов `(db as any).db`.
5. **Рефакторинг API Server**: Удалён `// @ts-nocheck` из `src/api/server.ts`, приведена в порядок типизация.
6. **Тесты**: Покрытие расширено до 18 unit-тестов Vitest (`MOCK_GEMINI=1 npm test` — 100% pass rate).

---

## 18. Поддержка структурированного JSON-вывода AGY CLI (2026-07-31)

Реализована интеграция с новым флагом `--output-format json`, появившимся в `agy` CLI v1.1.8+:

### Изменения:
1. **Адаптер `AgyCliClient` (`src/gemini/agy-client.ts`)**: При вызове `agy` передается флаг `--output-format json`. Добавлен парсинг структурированного ответа (поля `response` и `usage`) с фолбэком на plain text / ANSI stripping для обратной совместимости со старыми версиями CLI.
2. **Тесты (`tests/agy-client.test.ts`, `tests/fixtures/fake-agy.js`)**: Добавлены фикстура и unit-тест валидации разбора структурированного JSON.

---

## 19. Релиз версии v0.4.1 (2026-09-17) — Windows Resiliency & Stdin Piping

Портированы и адаптированы архитектурные решения из продакшен-проекта Polymarket Bot для стабильной работы `agy` CLI на Windows:

### Изменения:
1. **Передача промпта через `stdin` (`src/gemini/agy-client.ts`)**: Заменили `execFile` с флагом `-p` на потоковый запуск через `spawn`. Промпт пишется напрямую в `stdin` дочернего процесса. Полностью устранён лимит длины аргументов Windows (`CreateProcessW` 32 767 символов / `WinError 206`), снято искусственное ограничение усечения в 40K.
2. **Флаги безопасности и REPL**: Добавлены флаги `--dangerously-skip-permissions` (подавление интерактивных подтверждений прав и тулов) и `--disable-slash-commands`.
3. **Окружение без ANSI**: В `env` процесса принудительно добавлены `NO_COLOR=1` и `TERM=dumb`.
4. **Кроссплатформенный резолвер путей `resolveAgyPath()`**: Автоматическое нахождение `agy.exe` на Windows (включая `%LOCALAPPDATA%\agy\bin\agy.exe` и `where.exe`), корректное включение `{ shell: true }` только для `.cmd`/`.bat`. Поддержка вызова тестовых JS-фикстур через Node.
5. **Startup Watchdog**: Фоновый таймер отслеживает `--log-file` процесса и строку `Created conversation <uuid>`. При зависании процесса на этапе авторизации дольше 45 секунд процесс принудительно прерывается.
6. **Таймаут суммаризации**: Дефолтный `timeoutMs` увеличен до 120 000 мс (2 мин) для комфортной генерации длинных развёрнутых резюме моделями Gemini с thinking tokens.
7. **Кроссплатформенные тесты (`tests/paths.test.ts`)**: Привязка к Unix-слэшу заменена на `path.isAbsolute(result)`. Все 19 unit-тестов Vitest проходят успешно (100% pass rate).
